...
import apk.about.PlanarYUVLuminanceSource;
...
public class QRcodeReader extends Activity
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        try {
            ...
            //読み取った値をIDと名前に分割
            String res = new String(result.getText());
            String[] other = res.split(":", 0);
            int user_id = Integer.parseInt(other[0]);
            
            String user_name = other[1];
            //分割したIDと名前をSQLiteに追加
            ABOUT_DBHelper helper = new ABOUT_DBHelper(this);
            SQLiteDatabase db = helper.getWritableDatabase();
            ABOUT_DBHelper.insert_Other(db, user_id, user_name);
